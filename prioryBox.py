...
def generate_priory_box(img_size=300):
    """
    生成先验框，参考https://zhuanlan.zhihu.com/p/95032060的讲解
    :param img_size: 默认为300
    :return: 返回生成的先验框 [A,4] : [x_ctr,y_ctr,w,h]
    """
    min_size = [300 * (0.2 + (0.9 - 0.2) / (5) * (k - 1)) for k in range(1, len(FEATURE_MAP_SIZE) + 1)]
    max_size = [300 * (0.2 + (0.9 - 0.2) / (5) * (k - 1)) for k in range(2, len(FEATURE_MAP_SIZE) + 2)]
    boxes = []

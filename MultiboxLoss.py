...
class MultiboxLoss(nn.Module):
    ...
    def forward(self,pred_conf:torch.Tensor,pred_loc:torch.Tensor,priory_boxes:torch.Tensor,truth:torch.Tensor):
        ...
        for idx in range(num):
            ...
            self.match(truth[idx,:,:4],priory_boxes.cuda(),truth[idx,:,4],loc_t,conf_t,idx)
        if self.use_gpu:
            loc_t=loc_t.cuda()
            conf_t=conf_t.cuda()
        # 包装变量
        ...
        # 选出非背景框，即label>0的框
        pos=conf_t>0
        pos_idx=pos.unsqueeze(pos.dim()).expand_as(loc_t) # (batch_size,num_priory)->(batch_size,num_priory,1)->(batch_size,num_priory,4)
        # 获取这些框的坐标
    ...
    def match(self, truth_boxes: torch.Tensor, priory_boxes: torch.Tensor, labels: torch.Tensor,loc_t:torch.Tensor,conf_t:torch.Tensor,idx):
        """
        ...
        :return:
        """
        IOU=self.calculateIOU(self.transFromBoxToPoint(priory_boxes),truth_boxes)# 计算重叠部分比例
        # 为每一个gt框匹配一个最大IOU的先验框
        best_priory_overlap,best_priory_idx=torch.max(IOU,dim=1,keepdim=True)
        # 为每一个先验框匹配一个gt框
        best_gt_overlap,best_gt_idx=torch.max(IOU,dim=0,keepdim=True)
        ...
        conf[best_gt_overlap<THRESHHOLD]=0 # 小于阈值的设置为0

        # 取出每一个先验框对应的最佳GT框的坐标
        matched=truth_boxes[best_gt_idx]
    ...
    def calculateIOU(self, boxes_priory: torch.Tensor, boxes_gt: torch.Tensor)->torch.Tensor:
        ...
        # 计算交叉部分的面积
        max_xy,_=torch.min(boxes_priory[:,2:].unsqueeze(1).expand(A,B,2),boxes_gt[:,2:].unsqueeze(0).expand(A,B,2))# 右下角坐标的较小值
        min_xy,_=torch.max(boxes_priory[:,:2].unsqueeze(1).expand(A,B,2),boxes_gt[:,:2].unsqueeze(0).expand(A,B,2))# 左上角坐标的较大值
